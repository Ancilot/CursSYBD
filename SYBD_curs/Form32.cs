...
namespace SYBD_curs
{
    public partial class Form32 : Form
    {
        ...
        public Form32(Form parent, int id)
        {
            InitializeComponent();
            ...
            parentForm = parent;
            emploeID = id;
            this.FormClosing += Form32_FormClosing;
        }
        private void Form32_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            DialogResult res = MessageBox.Show(
                "Вы уверены, что хотите отменить добавление сотруднику смен и лицензий?",
                "Подтверждение",
            ...
            try
            {
                ...
                using (var transaction = conn.BeginTransaction())
                {
                    using (var cmd = new NpgsqlCommand(
                        "DELETE FROM curse.\"Employee_License\" WHERE \"ID_Employee\" = @id_employee",
                        conn, transaction))
                    {
                    ...
                    using (var cmd = new NpgsqlCommand(
                        "DELETE FROM curse.\"Employee_graphik\" WHERE \"ID_Employee\" = @id_employee",
                        conn, transaction))
                    {
                        cmd.Parameters.AddWithValue("id_employee", emploeID);
                        cmd.ExecuteNonQuery();
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var cmdSmen = new NpgsqlCommand(
                    "SELECT COUNT(*) FROM curse.\"Employee_graphik\" WHERE \"ID_Employee\" = @id",
                    conn
                );
                cmdSmen.Parameters.AddWithValue("@id", emploeID);
                int smenCount = Convert.ToInt32(cmdSmen.ExecuteScalar());

                // Проверка лицензий
                var cmdServices = new NpgsqlCommand(
                    "SELECT COUNT(*) FROM curse.\"Employee_License\" WHERE \"ID_Employee\" = @id",
                    conn
                );
                cmdServices.Parameters.AddWithValue("@id", emploeID);
                int servicesCount = Convert.ToInt32(cmdServices.ExecuteScalar());

                // Если нет хотя бы одной смены или лицензии
                if (smenCount == 0 && servicesCount == 0)
                {
                    DialogResult res = MessageBox.Show(
                        "Вы уверены, что хотите оставить сотрудника без смен и лицензий?",
                        "Подтверждение",
                        MessageBoxButtons.YesNo,
                        MessageBoxIcon.Warning
                    );
                    ...
                    if (res != DialogResult.Yes)
                    {
                        // Пользователь передумал — выходим из метода, формы НЕ закрываем
                        return;
                    }
                }
